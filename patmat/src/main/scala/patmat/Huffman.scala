...
trait Huffman extends HuffmanInterface {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    if (trees.size < 2){
      ...
      (makeCodeTree(trees(0), trees(1)) +: trees.drop(2)).sortBy(x=>weight(x))
    }
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
    def convertImpl(tree: CodeTree, acc: CodeTable): CodeTable = tree match {
      case tree: Fork => mergeCodeTables(convertImpl(tree.left, chars(tree.left).map(c => (c, codeBits(acc)(c) :+ 0))),
        convertImpl(tree.right, chars(tree.right).map(c => (c, codeBits(acc)(c) :+ 1))))
      case tree: Leaf => acc
    }
    convertImpl(tree, List[(Char, List[Bit])]())
  }
